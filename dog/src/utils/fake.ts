...
export const challengeList: IChallenge[] = Array.from({ length: 30 }, () => ({
  ...
  state: challengeStateList[Math.floor(Math.random() * challengeStateList.length)],
  category: challengeCategoryList[Math.floor(Math.random() * challengeCategoryList.length)],
}))
...
export const competitionList: ICompetition[] = Array.from({ length: 200 }, () => ({
  id: v4(),
  name: faker.name.findName(),
  ...
  tags: [],
  state: competitionStateList[Math.floor(Math.random() * competitionStateList.length)],
  authors: [],
