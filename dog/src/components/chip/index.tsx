...
const Chip: React.FC<Props> = ({ className = '', children, color = 'primary' }): React.ReactElement => {
  const classNames = `${colors[color]}`

  return <div className={`flex items-center rounded-3xl px-3 py-1 ${className} ${classNames}`}>{children}</div>
}
